...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        SFContentBlockerManager.getStateOfContentBlocker(withIdentifier: "Bhadasia.BlockIT.Blocker", completionHandler: { (state, error) in
            if let error = error {
                print(error)
            }
            if let state = state {
                let contentBlockerIsEnabled = state.isEnabled
                print("isEnable : \(contentBlockerIsEnabled)")
            }
        })
        return true
    }
